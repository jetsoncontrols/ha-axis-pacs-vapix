...
async def _async_register_frontend(hass: HomeAssistant) -> None:
    """Serve the bundled management card and auto-load it as an extra JS module.

    Ships the custom Lovelace card inside the integration (the browser_mod
    pattern), so installing the integration is enough — no HACS frontend entry
    or hand-registered Lovelace resource. Loading is idempotent across reloads.
    """
    frontend_dir = Path(__file__).parent / FRONTEND_DIR
    await hass.http.async_register_static_paths(
        [StaticPathConfig(FRONTEND_URL_BASE, str(frontend_dir), False)]
    )
    # Cache-bust with a content hash so a card update is always a fresh URL —
    # otherwise the frontend service worker can serve a stale/partial module
    # (intermittent "Configuration error" on the card). The static handler
    # ignores the query string and serves the file.
