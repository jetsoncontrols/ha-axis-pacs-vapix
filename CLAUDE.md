...
- **Delivery — MUST serve from a service-worker-cached path, i.e. `www/` → `/local/` (v0.4.3+, the actual fix).** `_async_register_frontend` copies the bundled `frontend/axis-pacs-codes-card.js` into `www/<domain>/` on every setup (`_deploy_card`, executor job — a HACS update thus propagates automatically, NO manual www management), then loads it from **`/local/axis_pacs/axis-pacs-codes-card.js?v=<hash>`** via BOTH `add_extra_js_url` AND an auto-managed **Lovelace resource** (`_async_register_card_resource`, same URL → the browser dedupes to one module load). `manifest.json` deps: `http, frontend, lovelace, websocket_api`.
