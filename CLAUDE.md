...
- **Delivery — TWO conditions, both required (final working design as of v0.4.4).** `_async_register_frontend` copies the bundled `frontend/axis-pacs-codes-card.js` into `www/<domain>/` on every setup (`_deploy_card`, executor job — a HACS update propagates automatically, NO manual www management), then loads it via BOTH `add_extra_js_url` AND an auto-managed **Lovelace resource** (`_async_register_card_resource`). `manifest.json` deps: `http, frontend, lovelace, websocket_api`.
  1. **(v0.4.3) Serve from a SERVICE-WORKER-CACHED path — `/local/` (i.e. `www/`), never the integration's own static path.** HA's SW only caches `api|static|auth|frontend_latest|frontend_es5|local`; `/axis_pacs/frontend` (0.3.0–0.4.2) was NOT cached, so Safari over Nabu Casa stalled the module request on load.
  2. **(v0.4.4) The two delivery URLs MUST DIFFER — `?v=<hash>` (extra_js_url) vs `?res=<hash>` (resource) — never identical.** Loading the SAME module URL via both `import()` and a `<script type=module>` trips a **module-map collision in Safari**: the module runs (`console.info("loaded")` fires) but `customElements.define` never registers the element (`customElements.get(...)` → undefined), so the card shows a permanent "Configuration error"; Chrome tolerates it. **0.4.1's "point both at the same URL so the browser dedupes" was itself the bug.** browser_mod does the same split (`?<ver>` vs `?automatically-added&<ver>`). Distinct queries = two independent module records; the card's `customElements.define` is idempotent-guarded so the 2nd load is a harmless no-op.
- **WHY `/local/` and not the integration's own static path (the whole 0.3.0→0.4.3 saga):** HA's **service worker only runtime-caches** `api | static | auth | frontend_latest | frontend_es5 | local`. A custom integration static path (`/axis_pacs/frontend`, what 0.3.0–0.4.2 used) is **NOT** on that list, so **Safari over a Nabu Casa tunnel stalls the non-SW-cached module request on page load** → the module never executes, the element never registers via `customElements.define`, and the card shows a permanent **"Configuration error"** until a full reload (HA's ~30s auto-reload, or a manual refresh; a manual `import()` in the console also fixed it instantly — that was the smoking gun). Chrome tolerated `/axis_pacs/`; Safari did not. `/local/` (www) IS SW-cached → loads reliably in **both** browsers (verified SI2 2026-07-01, 3/3 each). Every other custom card on the dash loads fine precisely because it's on an SW-cached path (`/hacsfiles/*`, `/browser_mod.js`).
- **Red herrings we chased first (don't repeat them):** (1) the **add_extra_js_url fire-and-forget `import()` race** — real, but not the cause here; (2) the **expander-card wrapper** — the card was moved to a top-level view card and STILL failed; (3) **`cache_headers=True` on the `/axis_pacs` static path (v0.4.2)** — not only failed to fix it, it was **HARMFUL**: the 31-day `max-age` let a stale/bad `/axis_pacs` response stick in Chrome's cache and broke Chrome too. Reverted in 0.4.3 (the `/axis_pacs` static path for the card is gone entirely). The 0.4.1 **resource registration + idempotent `customElements.define` guard are KEPT** — correct hardening, just never the fix.
- **Cache-bust:** the `/local/` URL carries `?v=<md5 of the file>` (from `_deploy_card`); a card update changes the bytes → new hash → new URL → SW/browser refetch. `_async_register_card_resource` rewrites the resource entry's `?v=` to the current hash on each setup (matched by URL prefix, ignoring the query), so a redeploy self-heals without a manual edit.
