...
- **Auto-registration** (`__init__.async_setup` → `_async_register_frontend`): `hass.http.async_register_static_paths([StaticPathConfig('/axis_pacs/frontend', <pkg>/frontend, False)])` + BOTH `frontend.add_extra_js_url(hass, '/axis_pacs/frontend/axis-pacs-codes-card.js?v=<hash>')` AND (v0.4.1+) a **Lovelace resource** with the SAME URL (`_async_register_card_resource`). The FULL browser_mod pattern — installing the integration is enough; no `www/` drop, and the resource is auto-managed (not hand-added). `manifest.json` deps: `http, frontend, lovelace, websocket_api`.
- **Why BOTH (the v0.4.1 cold-load fix):** `add_extra_js_url` alone becomes a **fire-and-forget `import()` in the app shell** (`<script>import("…iconset");import("/axis_pacs/frontend/…");import("/browser_mod.js")</script>`) that is NOT awaited — on a cold browser load the dashboard view can render before it resolves, so `customElements.get('axis-pacs-codes-card')` is undefined and HA paints **"Configuration error"** inside the (expander-wrapped) card; it takes several refreshes to win the race. Lovelace **resources** are loaded by the panel BEFORE it builds cards — which is exactly how every OTHER custom card on the dashboard (expander-card, card-mod, browser_mod) loads reliably; this card was the only one on the racy path. `_async_register_card_resource` mirrors browser_mod's `mod_view` (find by URL prefix → update the `?v=` hash in place or create; `ResourceStorageCollection`/storage-mode only, no-op + best-effort in YAML mode so it never breaks setup). The card's `customElements.define` is now **idempotent-guarded** so a stray second load can't throw "already defined".
- **Cache-bust:** the URL carries `?v=<md5 of the file>` (`_card_version`) so a card update changes the URL and the BROWSER's HTTP cache refetches (the static handler ignores the query and serves the current file, and sends ETag/Last-Modified). NOTE: this is NOT a service-worker concern — HA's SW only runtime-caches `api|static|auth|frontend_latest|frontend_es5|local`, never `/axis_pacs/frontend/` (verified on 2026.5). On setup, `_async_register_card_resource` rewrites the resource entry's `?v=` to the current hash, so a redeploy self-heals the resource URL without a manual edit.
